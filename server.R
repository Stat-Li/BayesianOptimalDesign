...
server <- function(input, output, session) {
  ...
  df_out <- eventReactive(input$go, {
    ...
    x_year <- input$x_year
    gamma_upper_bound <- input$search_gamma
    n_sims <- input$n_sims
    ...
    if(input$design_arm=="Single-arm"){
      ...
      for (h in 1:length(pram_gamma_a)) {
        ...
        pram_gamma_b = 2*pram_gamma_a[h]/(1 + delta0)
        ...
        p2 = seq (0.01, gamma_upper_bound, by = 0.01)
        ...
        if(input$design_stop == "Futility and superiority stopping"){
          ...
          tot_bound_fut = tot_stop_futility(gamma_a = pram_gamma_a[h], gamma_b = pram_gamma_b, 
                                        p_lambda = p_opt[1], p_gamma = p_opt[2], 
                                        t_interim = timing, event_tot = m_target)
          tot_bound_sup = tot_stop_superiority(gamma_a = pram_gamma_a[h], gamma_b = pram_gamma_b, 
                                            p_lambda = p_opt[1], 
                                            t_interim = timing, event_tot = m_target)
          results_stop = rbind(results_stop, cbind(tot_bound_fut, tot_bound_sup))
        ...
        if(input$design_stop == "Futility stopping only"){
          ...
          tot_bound = tot_stop_futility(gamma_a = pram_gamma_a[h], gamma_b = pram_gamma_b, 
                               p_lambda = p_opt[1], p_gamma = p_opt[2], 
                               t_interim = timing, event_tot = m_target)
          results_stop = rbind(results_stop, tot_bound)
      ...
      if(input$design_stop == "Futility stopping only"){
        ...
        names(df_final_stop) = c('Design', 'Gamma prior shape',
                                 'Information time', 'Stop trial if TOT (transformed) <')
      }
      if(input$design_stop == "Futility and superiority stopping"){
        df_final_ops = data.frame(cbind(rep(1:length(pram_gamma_a), each = 2), weibull_shape,
                                    rep(pram_gamma_a, each = 2), rep(c(1, delta_1), length(pram_gamma_a)),
        ...
        df_final_stop = data.frame(cbind(rep(1:length(pram_gamma_a), each = length(timing)),
                                         ...
                                         results_stop))
        names(df_final_stop) = c('Design', 'Gamma prior shape',
                                 'Information time', 'Stop for futility if TOT (transformed) <', 
                                 'Stop for superiority if TOT (transformed) >=')
      }
    }
    if(input$design_arm=="Two-arm RCT"){
      ...
      for (h in 1:length(sigma2_0)){
        ...
        if(input$design_stop == "Futility and superiority stopping"){
          ...
          p_cut_sup = cut_super(timing, p_lambda = p_opt[1])
          results_stop = rbind(results_stop, cbind(p_cut_fut, p_cut_sup))
          # results_tune = rbind(results_tune, c(weibull_shape, sigma2_0[h],  p_opt))
          PRN0 = grid_search_all[which.min(grid_search_all[,5]),1]
          PET0 = grid_search_all[which.min(grid_search_all[,5]),2]
          PET_EFS0 = grid_search_all[which.min(grid_search_all[,5]),3]
          PET_ESS0 = grid_search_all[which.min(grid_search_all[,5]),4]
    ...
    # zz=data.frame(cbind(n_max, clt_ratio))
    # names(zz)=c('x1 (%)','PET_f')
    # return(zz)
